...
Private.facebook.post.revert = function( req, connection ) {

	var own_callback = function( item_id, item_url ) {
		if( 'undefined' === typeof req.data.ids ) {
			req.data.ids = {};
		}
		req.data.ids.facebook = id;
	...
	var target_id = null;
	if( 'undefined' !== typeof req.data && 'undefined' !== typeof req.data.target && 'undefined' !== typeof req.data.target.ids ) {
       		target_id = req.data.target.ids.facebook;
	}
	...
	var callback = function( status_code, data ) {
		...
		status_code = data.statusCode;
		console.log( "FACEBOOK DELETE", data );
		if( 'function' === typeof own_callback ) {
			own_callback();
		}	
	};
...
Private.twitter.post.revert = function( req, connection ) {
	...
	if( 'comment' === req.type ) {
		...
		if( 'undefined' !== typeof query.target && 'undefined' !== typeof query.target.ids ) {
			target_id = query.target.ids.twitter;
		}
		...
		var callback = function( status_code, data ) {
			...
			console.log("TWITTER DESTROY STATUS", data);
			if( 'function' === typeof own_callback ) {
				own_callback( item_id, item_url );
			}	
		};
