...
Private.github.post = function( req ) {
	...
	var callback = function( data ) {
		...
		if( 'undefined' !== typeof data ) {
			console.log('t2', typeof data, data['history']);
			if( 'undefined' !== typeof data[ 'history' ] ) {
				console.log('t3',typeof data['history']);
				var histlen = data.history.length;
				for( z = 0; z < histlen; z += 1 ) {
					console.log('t4', z );
					if( 'undefined' !== typeof data.history[ z ] ) {
						console.log('t5',JSON.stringify(data.history[z]));
						if( 'undefined' !== typeof data.history[ z ].id ) {
							if( null === typeof item_id ) {
								item_id = data.history[ z ].id;
							} else if( 'string' === typeof item_id ) {
								item_id = [ item_id, data.history[ z ].id ];
							} else {
								item_id.push( data.history[ z ].id );
							}
						}
						if( 'undefined' !== typeof data.history[ z ].html_url ) {
							if( null === typeof item_url ) {
								item_url = data.history[ z ][ 'html_url' ];
							} else if( 'string' === typeof item_url ) {
								item_url = [ item_id, data.history[ z ][ 'html_url' ] ];
							} else {
								item_url.push( data.history[ 'html_url' ] );
							}
						}
					}
				}
