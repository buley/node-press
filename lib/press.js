...
Private.post = function( req, connection ) {
	var own_callback = function( id, url ) {
		...
		req.data.url = url;
		console.log("ONE",req);
		console.trace();connection.emit( Private.prefix, req );
	};
	req.actions = [ 'post' ];
	return this.apply_actions( req, connection );
};
...
Private.linkedin.post = function( req, connection ) {
	var own_callback = function( id, url ) {
		...
		if( complete ) {
			...
			console.log("WOULD HAVELOGGED: " +JSON.stringify(return_obj));
			//console.trace();connection.emit( Private.prefix, return_obj );
...
Public.prototype.request = function( request, connection, type ) {
	...
	if( 'socket' === type ) {
		...
		console.trace();connection.emit( Private.prefix, response );
	}
