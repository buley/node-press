		...
		if( 'undefined' !== typeof data ) {
			...
			if( 'undefined' !== typeof data[ 'files' ] ) {
				...
				for( filename in data.files ) {
					...
					if( 'undefined' !== typeof data[ attr ][ filename ] && 'undefined' !== typeof data[ attr ][ filename ].id ) {
						if( null === typeof item_id ) {
							item_id = data[ attr ][ filename ].id;
						} else if( 'string' === typeof data[ attr ] ) {
							item_id = [ item_id, data[ attr ][ filename ].id ];
						} else {
							item_id.push( data[ attr ][ filename ].id );
						}
						if( null === typeof item_url ) {
							item_url = data[ attr ][ filename ][ 'html_url' ];
						} else if( 'string' === typeof data[ attr ] ) {
							item_url = [ item_id, data[ attr ][ filename ][ 'html_url' ] ];
						} else {
							item_url.push( data[ attr ][ filename ][ 'html_url' ] );
						}
					}
				}
			}
