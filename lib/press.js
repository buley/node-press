...
Private.tumblr.stream = function( req, connection ) {
	...
	var own_callback = function( data ) {
		if( null === req.data || 'undefined' === req.data || null === req.data.streams || 'undefined' === typeof req.data.streams ) {
			req.data.streams = {};
		}
		...
		for( x = 0; x < xlen; x += 1 ) {
			...
			if( null === req.data || 'undefined' === req.data || null === req.data.streams || 'undefined' === typeof req.data.streams[ service ] ) {
				complete = false;
			}
		}
		if( complete ) {
