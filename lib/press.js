	...
	var own_callback = function( data ) {
		...
		if( true === req.raw ) {
			...
			for( z = 0; z < zlen; z += 1 ) {
				...
				if(  'undefined' !== typeof zitem.updateContent.person.currentShare.content && 'undefined' !== typeof zitem.updateContent.person.currentShare.content.submittedUrl  ) {
					//link
					res.push( Private.stream.linkedin.link.to.activity( zitem ) );
				} else if(  'undefined' !== typeof zitem.updateContent.person.currentShare && 'undefined' !== typeof zitem.updateContent.person.currentShare.comment ) {
					//comment (status)
