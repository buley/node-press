...
Private.yahoo.post = function( req ) {
	...
	if( 'comment' === req.type ) {
		...
		var target_id = '';
		if( 'undefined' !== typeof req.target && 'undefined' !== typeof req.target.id ) {
		...
		var postbody = Private.request.querystring( query );
		var callback = function( status_code, data ) {
			console.log( 'Yahoo post (status) posted' );
			console.log( 'STATUS CODE: ' + JSON.stringify( status_code ) );
			console.log( 'STRING: ' + JSON.stringify( data ) );
		};
...
Private.linkedin.post = function( req ) {
	...
	if( 'comment' === req.type ) {
		...
		console.log('is there a target id?', req.target );
		if( 'undefined' === type req.target || 'undefined' === typeof req.target.id || 'group' === req.target.type ) {
			console.log('must defined a target group and make the target type "group"',req);
			return false;
		}
		query = Private.models.linkedin.note( req.data );
		url = 'http://api.linkedin.com/v1/groups/' + req.target.id + '/posts';
	}
