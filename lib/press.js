...
Private.stream = function( req, connection ) {
	var own_callback = function( id, url ) {
		connection.emit( Private.prefix, {} );
	};
	req.actions = [ 'stream' ];
...
Private.tumblr.stream = function( req, connection ) {
...
Private.tumblr.favorite = function( req, connection, invert ) {
	...
	if( 'undefined' === typeof req.data || 'undefined' === typeof req.data.target || 'undefined' === typeof req.data.target.tumblr || 'undefined' === typeof req.data.target.tumblr.id || 'undefined' === typeof req.data.target.tumblr.id || 'undefined' === typeof req.data.target.tumblr.key ) {
		console.log("need to target a blog, have an id and key (reblog_key)");
	} else {
		target_blog = req.data.target.tumblr.id;
...
Private.tumblr.post = function( req, connection ) { 
	...
	if( 'undefined' === typeof req.data || 'undefined' === typeof req.data.target || 'undefined' === typeof req.data.target.tumblr || 'undefined' === typeof req.data.target.tumblr.id ) {
		console.log('missing target.tumblr.id');
		return false;
		//todo: new 
	}
	var url, query;
	if( 'note' === req.data.type ) {
		if( 'undefined' === typeof req.data.target || 'undefined' === typeof req.data.target.tumblr || 'undefined' === typeof req.data.target.tumblr.id ) {
			console.log("need to target a id (req.data.target.tumblr.id) ");
			return false;
		}
		url = 'http://api.tumblr.com/v2/blog/' + req.data.target.tumblr.id + '/post';
		query = Private.to.tumblr.note( req.data );
	} else if( 'comment' === req.data.type ) {
		if( 'undefined' !== typeof req.data.target && 'undefined' !== typeof req.data.target.tumblr && 'undefined' !== typeof req.data.target.tumblr.id ) {
			url = 'http://api.tumblr.com/v2/blog/' + req.data.target.tumblr.id + '/reblog';
			query = Private.to.tumblr.comment( req.data );
		}
	} else if( 'link' === req.data.type ) {
		if( 'undefined' !== typeof req.data.taget && ( 'undefined' === typeof req.data.target.tumblr || 'undefined' === typeof req.data.target.tumblr.id ) ) {
	...
	var callback = function( status_code,data ) {
		...
		console.log("GETTING",'http://api.tumblr.com/v2/blog/' + req.data.target.tumblr.id + '/posts?id=' + item_id);
		var res2 = Private.request.get( Private.service.tumblr, 'http://api.tumblr.com/v2/blog/' + req.data.target.tumblr.id + '/posts?id=' + item_id + '&api_key=' + tumblrId, token, secret, function(res2,res3) {
...
Private.tumblr.unpost = function( req, connection ) {
	...
	if( 'undefined' === typeof req.data || 'undefined' === typeof req.data.target || 'undefined' === typeof req.data.target.tumblr || 'undefined' === typeof req.data.target.tumblr.id || 'udnefined' === typeof req.data.target.tumblr.id ) {
		console.log("need to target a blog (target.ids.tumblr and target.types.tumblr)");
	} else {
		target_blog = req.data.target.tumblr.id;
		target_id = req.data.target.tumblr.id;
...
Private.facebook.stream = function( req, connection ) {
...
Private.twitter.stream = function( req, connection ) {
...
Private.github.stream = function( req, connection ) {
...
Private.google.stream = function( req, connection ) {
...
Private.yahoo.stream = function( req, connection ) {
...
//not yet supported
Private.windows.stream = function( req, connection ) {
	return false;
	/*
	var own_callback = function( data ) {
		if( null === req.data || 'undefined' === typeof req.data ) {
...
Private.foursquare.stream = function( req, connection ) {
...
Private.linkedin.stream = function( req, connection ) {
...
Private.linkedin.post = function( req, connection ) {
	...
	if( 'comment' === req.data.type ) {
		...
		|| 'undefined' === typeof req.data.target.linkedin.id ) {
		...
		url = 'http://api.linkedin.com/v1/groups/' + req.data.target.linkedin.id + '/posts';
	}
	...
	var callback = function( status_code, data ) {
		...
		var item_url = null;
		if ( 'note' === req.data.type ) {
			var url2 = 'http://api.linkedin.com/v1/groups/' + req.data.target.linkedin.id + '/posts:(site-group-post-url,id,creation-timestamp,title,summary,creator:(first-name,last-name,picture-url,headline),likes,attachment:(image-url,content-domain,content-url,title,summary),relation-to-viewer)?category=discussion&order=recency&count=5';
...
Public.prototype.stream = function( req, connection ) {
