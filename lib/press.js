...
Private.github.post = function( req ) {
	...
	var callback = function( data ) {
		...
		if( 'undefined' !== typeof data ) {
			console.log('t2', typeof data, data['files']);
			if( 'undefined' !== typeof data[ 'files' ] ) {
				console.log('t3',typeof data['files']);
				for( filename in data.files ) {
					console.log('t4', filename );
					if( 'undefined' !== typeof data.files[ filename ] ) {
						console.log('t5',JSON.stringify(data.files[filename]));
						if( 'undefined' !== typeof data.files[ filename ].id ) {
							if( null === typeof item_id ) {
								item_id = data.files[ filename ].id;
							} else if( 'string' === typeof item_id ) {
								item_id = [ item_id, data.files[ filename ].id ];
							} else {
								item_id.push( data.files[ filename ].id );
							}
						}
						if( 'undefined' !== typeof data.files[ filename ].html_url ) {
							if( null === typeof item_url ) {
								item_url = data.files[ filename ][ 'html_url' ];
							} else if( 'string' === typeof item_url ) {
								item_url = [ item_id, data.files[ filename ][ 'html_url' ] ];
							} else {
								item_url.push( data.files[ 'html_url' ] );
							}
						}
					}
				}
