...
Private.twitter.stream = function( req, connection ) {
	...
	var own_callback = function( data ) {
		...
		if( null === req.data.streams || 'undefined' === typeof req.data.streams ) {
			req.data.streams = {};
		}
		...
		if( true === req.raw ) {
			...
			for( z = 0; z < zlen; z += 1 ) {
				zitem = data[ z ];
				console.log("TRANSLATING ITEM",zitem);
	...
	var callback = function( status_code, data ) {
		...
		console.log('calling own callback',typeof own_callback);
		if( 'function' === typeof own_callback ) {
			own_callback( data );
		}
	};
...
Private.stream.twitter.comment.to.activity = function( data ) {
	...
	console.log('Private.stream.map.tweet', data.user );
	console.log('Private.stream.map.tweet name display', data.user.screen_name, data.user.name );
	...
	console.log("AS OBJ",typeof AS,AS);
	var activ = AS.activity( request );
	console.log("RESULTING ACTIVITY",activ);
	return activ;
};
