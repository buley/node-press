...
Private.tumblr.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.tumblr, , token, secret, callback );
...
Private.facebook.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.facebook, , token, callback );
	...
	var callback = function( status_code, data ) {
		...
		if( 'undefined' !== typeof data ) {
			...
			if( 'undefined' !== typeof data.id ) {
				...
				var  = 'https://graph.facebook.com/me';	
				Private.request.get( Private.service.facebook, , token, function( status_code_3, data_3) {
...
Private.google.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.google, , token, callback );
...
Private.yahoo.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.yahoo, , token, secret, callback );
...
Private.windows.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.windows, , token, callback );
	*/
...
Private.foursquare.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.foursquare, , token, callback );
...
Private.linkedin.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.linkedin, url, token, callback, { 'x-li-format': 'json' } );
