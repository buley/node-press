...
Private.linkedin.post = function( req ) {
	...
	var callback = function( status_code, data ) {
		...
		if ( 'note' === req.type ) {
			...
			var url2 = 'http://api.linkedin.com/v1/groups/' + req.target.ids.linkedin + '/posts:(site-group-post-url,creation-timestamp,title,summary,creator:(first-name,last-name,picture-url,headline),likes,attachment:(image-url,content-domain,content-url,title,summary),relation-to-viewer)?category=discussion&order=recency&count=5';
