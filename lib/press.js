...
Private.linkedin.post = function( req ) {
	...
	var callback = function( status_code, data ) {
		...
		var res2 = Private.request.get( Private.service.linkedin, url2, token, secret, function( status_code2, data2 ) {
			...
			for( x = 0; x < xlen; x += 1 ) {
				item = data2.values[ x ];
				console.log("NEW",typeof item,JSON.stringify(item.updateContent),typeof item.updateContent.currentStatus);
				var orig = req.data.text.split( ' ' )[ 0 ];
				var newer = item.person.updateContent.currentStatus.split( ' ' )[ 0 ];
