...
Private.post = function( req, connection ) {
	var own_callback = function( id, url ) {
		req.data.id = id;
		req.data.url = url;
		connection.emit( Private.prefix, req );
	};
	req.actions = [ 'post' ];
	return this.apply_actions( req, connection );
};
...
Private.tumblr.post = function( req, connection ) { 
	var own_callback = function( id, url ) {
		...
		req.data.upstream_duplicates.tumblr = id;
		if( 'undefined' !== typeof req.meta && 'undefined' !== typeof req.meta.id ) {
			req.data.upstream_duplicates.newsgregator = req.meta.id;
		}
		...
		console.log("TUM IDS",req.data.upstream_duplicates);
		if( complete ) {
			connection.emit( Private.prefix, req );
		}
	};
...
Private.facebook.ownStream = function( req, connection ) {
...
Private.facebook.post = function( req, connection ) {
	var own_callback = function( id, url ) {
		...
		if( 'undefined' === typeof req.data.upstream_duplicates.newsgregator ) {
			req.data.upstream_duplicates.newsgregator = {};
		}
		if( 'undefined' !== typeof req.meta && 'undefined' !== typeof req.meta.id ) {
			req.data.upstream_duplicates.newsgregator.id = req.meta.id;
		}
		if( 'undefined' === typeof req.data.urls ) {
			req.data.urls = {};
		}
		...
		console.log("FB IDS",req.data.upstream_duplicates);
		if( complete ) {
			connection.emit( Private.prefix, req );
		}
	};
...
Private.twitter.post = function( req, connection ) {
	var own_callback = function( id, url ) {
		...
		req.data.upstream_duplicates.twitter = id;
		if( 'undefined' === typeof req.data.urls ) {
			req.data.urls = {};
		}
...
Private.github.post = function( req, connection ) {
	var own_callback = function( id, url ) {
		...
		req.data.upstream_duplicates.github = id;
		if( 'undefined' === typeof req.data.urls ) {
			req.data.urls = {};
		}
...
Private.yahoo.post = function( req, connection ) {
	var own_callback = function( id, url ) {
		...
		req.data.upstream_duplicates.yahoo = id;
		if( 'undefined' === typeof req.data.urls ) {
			req.data.urls = {};
		}
...
Private.windows.post = function( req, connection ) { 
	...
	var own_callback = function( id, url ) {
		...
		req.data.upstream_duplicates.windows = id;
		if( 'undefined' === typeof req.data.urls ) {
			req.data.urls = {};
		}
...
Private.linkedin.post = function( req, connection ) {
	var own_callback = function( id, url ) {
		...
		if( complete ) {
			...
			connection.emit( Private.prefix, return_obj );
		}
	};
	//https://developer.linkedin.com/documents/share-api
