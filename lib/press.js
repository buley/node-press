...
Private.models.tumblr.note = function( note ) {
	var result = {
		body: ( Private.emptyPath( 'text', organization ) ) ? null :  organization.text
		, title: ( Private.emptyPath( 'title', comment ) ) ? null :  organization.title
	};
	return result;
};
...
Private.models.linkedin.note = function( note ) {
	return  { title: ( Private.emptyPath( 'title', link ) ) ? null : link.title
		, summary: ( Private.emptyPath( 'text', link ) ) ? null : link.text
	        };
};
...
Private.tumblr.post = function( req ) { 
	...
	//http://www.tumblr.com/docs/en/api/v2#posts
	console.log('does it have a hostname?',req.target.hostname);
	console.log('tumblr',req.tokens.tumblr);
	console.log('action object',req.data);

	var url, query;
	if( 'note' === req.type ) {
		if( 'undefined' === typeof req.target.ids || 'undefined' === typeof req.target.ids.tumblr || 'blog' !=== req.target.types.tumblr ) {
			console.log("need to target a blog (target.ids.tumblr and target.types.tumblr)");
		}
		url = 'https://api.tumblr.com/v2/blog/' + req.target.ids.tumblr + '/post';
		query = Private.models.facebook.note( req.data );
	} else if( 'comment' === req.type ) {
		if( 'undefined' !== typeof req.target && 'undefined' !== typeof req.target.ids && 'undefined' !== typeof req.target.ids.tumblr ) {
			url = 'https://api.tumblr.com/v2/blog/' + req.target.ids.tumblr + '/reblog';
			query = Private.models.tumblr.comment( req.data );
		}
		...
		url = 'https://api.tumblr.com/v2/blog/' + req.target.ids.tumblr + '/post';
		query = Private.models.facebook.link( req.data );
	}

	var type = req.type;
	var hostname = req.target.hostname;
	...
	var callback = function( status_code, data ) {
		console.log( 'Tumblr post (gist) posted' );
       		console.log( 'STATUS CODE: ' + JSON.stringify( status_code ) );
		console.log( 'STRING: ' + JSON.stringify( data ) );
	};

	console.log("DOING POST",typeof Private.service.tumblr, url, token, secret, postbody, '', callback);

	var res = Private.request.post( Private.service.tumblr, url, token, postbody, null, callback );
...
Private.facebook.post = function( req ) {
	...
	if( 'note' === req.type ) {
		...
		query = Private.models.facebook.comment( req.data );
	} else if( 'link' === req.type ) {
...
Private.github.post = function( req ) {
	...
	console.log('action object',req.data);
	var query = Private.models.github.note( req.data );
	console.log('translated object',query);
	...
	var postbody = Private.request.querystring( query );
	var callback = function( status_code, data ) {
		console.log( 'Github post (gist) posted' );
       		console.log( 'STATUS CODE: ' + JSON.stringify( status_code ) );
		console.log( 'STRING: ' + JSON.stringify( data ) );
	};

	console.log("DOING POST",typeof Private.service.github, url, token, secret, postbody, '', callback);

	var res = Private.request.post( Private.service.github, url, token, postbody, null, callback );
	console.log("RESSSSULT", res);
...
Private.yahoo.post = function( req ) {
	...
	console.log('action object',req.data);
	if( 'comment' === req.type ) {
		var query = Private.models.yahoo.comment( req.data );
		console.log('translated object',query);
		console.log('is there a target id?', req.target );
		var target_id = '';
		if( 'undefined' !== typeof req.target && 'undefined' !== typeof req.target.ids ) {
			target_id = req.target.ids.yahoo;
		} else {
		...
		var postbody = Private.request.querystring( query );
		var callback = function( status_code, data ) {
			console.log( 'Yahoo post comment (status) posted' );
			console.log( 'STATUS CODE: ' + JSON.stringify( status_code ) );
			console.log( 'STRING: ' + JSON.stringify( data ) );
		};

		console.log("DOING POST",typeof Private.service.yahoo, url, token, secret, postbody, '', callback);

		var res = Private.request.post( Private.service.yahoo, url, token, postbody, null, callback );
		console.log("RESSSSULT", res);
...
Private.windows.post = function( req ) { 
	...
	var query;
	if( 'comment' === req.type ) {
		query = Private.models.windows.comment( req.data );
	} else if ( 'link' === req.type ) {
		query = Private.models.windows.link( req.data );
	}
	...
	var postbody = Private.request.querystring( query );
	var callback = function( status_code, data ) {
		console.log( 'windows post (status) posted' );
		console.log( 'STATUS CODE: ' + JSON.stringify( status_code ) );
		console.log( 'STRING: ' + JSON.stringify( data ) );
	};

	console.log("DOING POST",typeof Private.service.windows, url, token, secret, postbody, '', callback);

	var res = Private.request.post( Private.service.windows, url, token, postbody, null, callback );
	console.log("RESSSSULT", res);
...
Private.linkedin.post = function( req ) {
	...
	if( 'comment' === req.type ) {
		query = Private.models.linkedin.comment( req.data );
		url = 'https://api.linkedin.com/v1/people/~/shares';
	} else if ( 'link' === req.type ) {
		query = Private.models.linkedin.link( req.data );
		url = 'https://api.linkedin.com/v1/people/~/shares';
	} else if ( 'note' === req.type ) {
		console.log('is there a target id?', req.target );
		if( 'undefined' === type req.target || 'undefined' === typeof req.target.ids || 'group' === req.target.type ) {
			console.log('must defined a target group and make the target type "group"',req);
			return false;
		}
		query = Private.models.linkedin.note( req.data );
	...
	console.log("DOING POST",typeof Private.service.linkedin, url, token, secret, postbody, '', callback);

	var res = Private.request.post( Private.service.linkedin, url, token, postbody, null, callback );
	console.log("RESSSSULT", res);
