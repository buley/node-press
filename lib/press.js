...
Private.request.get = function( oauth, url, oauth_token, oauth_token_secret, callback, extra_headers ) {
	...
	if( 'function' !== typeof callback && 'function' === oauth_token_secret ) {S
		...
		return oauth.get( url, oauth_token, callback );
	} else {
		//oauth1.0
...
Private.tumblr.stream = function( req, connection ) {
	...
	var own_callback = function( data ) {
		if( 'undefined' === typeof req.data.streams ) {
			req.data.streams = {};
		}
		...
		for( x = 0; x < xlen; x += 1 ) {
			...
			if( 'undefined' === typeof req.data.streams[ service ] ) {
				complete = false;
			}
		}
		if( complete ) {
	...
	var query;
	var url = 'https://api.tumblr.com/v2/user/dashboard';	
	...
	Private.request.get( Private.service.tumblr, profile_url, token, secret, callback );
...
Private.facebook.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.facebook, profile_url, token, callback );
	...
	var callback = function( status_code, data ) {
		...
		if( 'undefined' !== typeof data ) {
			...
			if( 'undefined' !== typeof data.id ) {
				...
				var profile_url = 'https://graph.facebook.com/me';	
				Private.request.get( Private.service.facebook, profile_url, token, function( status_code_3, data_3) {
...
Private.twitter.stream = function( req, connection ) {
	...
	var callback = function( status_code, data ) {
		...
		console.log('twitter return data',data);
		console.log('justincase status code',status_code);
		if( 'string' === typeof data ) {
			data = JSON.parse( data );
		}
	...
	Private.request.get( Private.service.twitter, profile_url, token, secret, callback );
...
Private.github.stream = function( req, connection ) {
	...
	var query = req.data;
	if( 'undefined' === typeof query || 'undefined' === typeof query.ids || 'undefined' === typeof query.ids.github ) {
		return;
	}	
	var user_id = query.ids.github;

	var url = 'https://api.github.com/v3/users/' + user_id + '/received_events';	
	...
	var callback = function( status_code, data ) {
		...
		console.log('github return data',data);
		console.log('justincase status code',status_code);
		if( 'string' === typeof data ) {
			data = JSON.parse( data );
		}
	...
	Private.request.get( Private.service.github, profile_url, token, callback );
...
Private.google.stream = function( req, connection, own ) {
	...
	Private.request.get( Private.service.google, profile_url, token, callback );
...
Private.yahoo.stream = function( req, connection ) {
	...
	var query = req.data;
	if( 'undefined' === typeof query || 'undefined' === typeof query.ids || 'undefined' === typeof query.ids.yahoo ) {
		console.log( "Need to pass a data.ids.yahoo string" );
		return false;
	}	
	var user_id = query.ids.yahoo;
	...
	Private.request.get( Private.service.yahoo, profile_url, token, secret, callback );
...
Private.windows.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.windows, profile_url, token, callback );
	*/
...
Private.foursquare.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.foursquare, profile_url, token, callback );
...
Private.linkedin.stream = function( req, connection ) {
	...
	Private.request.get( Private.service.linkedin, profile_url, token, callback, { 'x-li-format': 'json' } );
