	...
	var own_callback = function( data ) {
		...
		if( true === req.raw ) {
			...
			for( z = 0; z < zlen; z += 1 ) {
				...
				if( 'undefined' !== typeof zitem.updateContent.person.currentShare.content.submittedUrl  ) {
					//link
					res.push( Private.stream.linkedin.link.to.activity( zitem ) );
				} else if( 'undefined' !== typeof zitem.updateContent.person.currentShare.comment ) {
					//comment (status)
...
Private.stream.facebook.comment.to.activity = function( data ) {
	...
	var request = {
		...
		'target_id': data.id,
		'target_content': data.message,
		'target_type': 'comment',
		'verb': 'post',
		'target_published': data.created_time,
		'target_updated': data.updated_time,
...
Private.stream.facebook.link.to.activity = function( data ) {
	...
	var request = {
		...
		'actor_image_url': 'https://graph.facebook.com/' + data.from.id + '/picture',
		'target_id': data.id,
		'target_display_name': data.name,
		'target_summary': data.description,
		'target_content': data.message,
		'target_image_url': data.picture,
		'target_type': 'link',
		'verb': 'post',
		'target_published': data.created_time,
		'target_updated': data.updated_time,
...
Private.stream.tumblr.link.to.activity = function( data ) {
	...
	var request = {
		...
		'target_display_name': data.title,
		'target_summary': data.description,
		'target_type': 'link',
		'verb': 'post',
...
Private.stream.linkedin.link.to.activity = function( data ) {
	...
	var request = {
		...
		'author_url': data.updateContent.person.currentShare.siteStandardProfileRequest.url,
		'object_id': data.updateContent.person.currentShare.id,
		'target_url': data.updateContent.person.currentShare.content.submittedUrl,
		'target_display_name': data.updateContent.person.currentShare.content.title,
		'target_description': data.updateContent.person.currentShare.content.description,
		'target_content': data.updateContent.person.currentShare.comment,
		'target_image_url': data.updateContent.person.currentShare.content.submittedImageUrl,
		'target_type': 'link',
		'verb': 'post',
...
Private.stream.linkedin.note = Private.stream.linkedin.note || {};
Private.stream.linkedin.note.to = Private.stream.linkedin.note.to || {};
...
Private.stream.linkedin.comment.to.activity = function( data ) {
	...
	var request = {
		...
		'actor_id': data.updateContent.person.currentShare.author.id,
		'author_url': data.updateContent.person.currentShare.siteStandardProfileRequest.url,
		'author_image_url': data.updateContent.person.currentShare.pictureUrl,
		'object_id': data.updateContent.person.currentShare.id,
		'target_url': data.post_url,
		'target_display_name': data.title,
		'target_content': data.updateContent.person.currentShare.comment,
