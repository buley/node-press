...
Private.linkedin.post = function( req ) {
	...
	var callback = function( status_code, data ) {
		...
		if ( 'note' === req.type ) {
			var url2 = 'http://api.linkedin.com/v1/people/~/group-memberships/{group-id}/posts:(creation-timestamp,title,summary,creator:(first-name,last-name,picture-url,headline),likes,attachment:(image-url,content-domain,content-url,title,summary),relation-to-viewer)?role=creator&category=discussion';
