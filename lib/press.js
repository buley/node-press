	...
	for( attr in obj ) {
		if( obj.hasOwnProperty( attr ) ) {
			if( 0 === inc ) {
				...
				str = str + '&' + attr + '=' + obj[ attr ].toString();
			}
			inc += 1;
		}
	};
...
Private.twitter.post = function( req ) {
	...
	var url = 'https://api.twitter.com/1/favorites/create/171765955117064192';
	...
	//var res = Private.request.get( Private.service.twitter, url, token, secret, callback );
	var res1 = Private.service.twitter.post( url, token, secret, { 'status': 'only a test' }, null, callback );
	//var res = Private.service.twitter._performSecureRequest( token, secret, "POST", url, null, postbody, null, callback );
