...
Private.twitter.post = function( req ) {
	...
	if( 'comment' === req.type ) {
		...
		var postbody = Private.request.querystring( query );
		var callback = function( status_code, req ) {
			if( 'string' === typeof req ) {
				req = JSON.parse( req );
			}
			var data = req.data;
...
Private.yahoo.post = function( req ) {
	...
	if( 'comment' === req.type ) {
		...
		var callback = function( req ) {
			...
			var data = req.data, status_code = req.status_code;
			var res = Private.request.put( Private.service.yahoo, url, token, secret, postbody, 'application/json', function( req2 ) {
				var data2 = req2.data, status_code = req2.status_code;
