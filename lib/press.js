...
Private.favorite.revert = function( req, connection ) {
	req.callback = function( id, url ) {
		//connection.emit( Private.prefix, {} );
	};
	req.actions = [ 'favorite' ];
...
Private.post.revert = function( req, connection ) {
	req.callback = function( id, url ) {
		req.data.id = id;
		req.data.url = url;
		connection.emit( Private.prefix, req.data );
...
Private.tumblr.favorite = function( req, connection ) {
	// /user/like
	//http://www.tumblr.com/docs/en/api/v2#posts
	return false; };

Private.tumblr.favorite.revert = function( req, connection ) {
	// /user/unlike
	//http://www.tumblr.com/docs/en/api/v2#posts
	return false; };
...
Private.tumblr.post = function( req, connection ) { 
	...
	if( 'note' === req.type ) {
		...
		if( 'undefined' !== typeof req.data.taget && ( 'undefined' === typeof req.data.target.ids || 'undefined' === typeof req.data.target.ids.tumblr ) ) {
			console.log("need to target a blog (target.ids.tumblr and target.types.tumblr)");
		}
		url = 'http://api.tumblr.com/v2/blog/' + req.data.target.ids.tumblr + '/post';
...
Private.tumblr.post.revert = function( req, connection ) {
	//delete
	//http://www.tumblr.com/docs/en/api/v2#posts
	return false; };
...
Private.facebook.favorite = function( req, connection ) {
	//https://developers.facebook.com/docs/opengraph/actions/
	return false;
};
Private.facebook.favorite.revert = function( req, connection ) { return false; };
...
Private.facebook.post.revert = function( req, connection ) { return false; };
...
Private.twitter.favorite.revert = function( req, connection ) {
};
...
Private.twitter.post.revert = function( req, connection ) {
...
Private.github.favorite = function( req, connection ) { return false; };

Private.github.favorite.revert = function( req, connection ) { return false; };
...
Private.github.post.revert = function( req, connection ) {
...
Private.google.favorite.revert = function( req, connection ) { return false; };
...
Private.google.post.revert = function( req, connection ) { return false; };
...
Private.yahoo.favorite.revert = function( req, connection ) { return false; };
	...
	if( 'comment' === req.type ) {
		...
		var callback = function( status_code, data ) {
			...
			var res = Private.request.get( Private.service.yahoo, url, token, secret, function( status2, data2 ) {
				...
				if( 'function' === typeof req.callback ) {
					req.callback( item_id2, item_url2 );
				}
...
Private.yahoo.post.revert = function( req, connection ) { return false; };
...
Private.windows.favorite.revert = function( req, connection ) { return false; };
...
Private.windows.post.revert = function( req, connection ) { return false; };
...
Private.foursquare.favorite.revert = function( req, connection ) { return false; };
...
Private.foursquare.post.revert = function( req, connection ) {
...
Private.linkedin.favorite.revert = function( req, connection ) { return false; };
...
Private.linkedin.post = function( req, connection ) {
	...
	if( 'comment' === req.type ) {
		...
		if( 'undefined' === typeof req.target || 'undefined' === typeof req.target.ids || ( 'undefined' !== typeof req.target.types && 'group' !== req.target.types.linkedin ) || 'undefined' === typeof req.target.ids.linkedin ) {
	...
	var callback = function( status_code, data ) {
		if( 'string' === typeof data ) {
			if( '' === data.replace(/\s/g, '' ) ) {
				...
				console.log("DATA FOR LINKEDIN",data);
				if( data.match( '<error>' ) ) {
					if( 'function' === typeof req.callback ) {
		...
		var item_url = null;
		if ( 'note' === req.type ) {
			//var url2 = 'http://api.linkedin.com/v1/people/~/group-memberships/' + req.target.ids.linkedin + '/posts:(creation-timestamp,title,summary,creator:(first-name,last-name,picture-url,headline),likes,attachment:(image-url,content-domain,content-url,title,summary),relation-to-viewer)?role=creator&category=discussion';
			var url2 = 'http://api.linkedin.com/v1/groups/' + req.target.ids.linkedin + '/posts:(site-group-post-url,id,creation-timestamp,title,summary,creator:(first-name,last-name,picture-url,headline),likes,attachment:(image-url,content-domain,content-url,title,summary),relation-to-viewer)?category=discussion&order=recency&count=5';
			var res2 = Private.request.get( Private.service.linkedin, url2, token, secret, function( status_code2, data2 ) {
				...
				if( 'function' === typeof req.callback ) {
					return req.callback( item_id, item_url );
				}	
			}, { 'x-li-format': 'json' } );
		} else {
			...
			var res2 = Private.request.get( Private.service.linkedin, url2, token, secret, function( status_code2, data2 ) {
				...
				for( x = 0; x < xlen; x += 1 ) {
					item = data2.values[ x ];
					console.log("NEW",typeof item,JSON.stringify(item.updateContent.person.currentShare.comment));
...
Private.linkedin.post.revert = function( req, connection ) { return false; };
