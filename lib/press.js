...
Private.request.post = function( oauth, url, oauth_token, oauth_token_secret, post_body, post_content_type, callback ) {
	console.log( 'post', url, oauth_token, oauth_token_secret, post_body, post_content_type, typeof callback );
...
Private.tumblr.post = function( req ) { 
	...
	var url, query;
	if( 'note' === req.type ) {
		if( 'undefined' === typeof req.target.ids || 'undefined' === typeof req.target.ids.tumblr ) {
			console.log("need to target a blog (target.ids.tumblr and target.types.tumblr)");
		}
		url = 'http://api.tumblr.com/v2/blog/' + req.target.ids.tumblr + '/post';
		...
		if( 'undefined' !== typeof req.target && 'undefined' !== typeof req.target.ids && 'undefined' !== typeof req.target.ids.tumblr ) {
			url = 'http://api.tumblr.com/v2/blog/' + req.target.ids.tumblr + '/reblog';
	...
	var callback = function( status_code, data ) {
		console.log( 'Tumblr post posted' );
...
Private.github.post = function( req ) {
	...
	var res = Private.request.post( Private.service.github, url, token, secret, postbody, null, callback );
	console.log("RESSSSULT", res);
...
Private.yahoo.post = function( req ) {
	...
	if( 'comment' === req.type ) {
		...
		var res = Private.request.post( Private.service.yahoo, url, token, secret, postbody, null, callback );
		console.log("RESSSSULT", res);
...
Private.windows.post = function( req ) { 
	...
	var res = Private.request.post( Private.service.windows, url, token, secret, postbody, null, callback );
	console.log("RESSSSULT", res);
...
Private.linkedin.post = function( req ) {
	...
	if( 'comment' === req.type ) {
		...
		console.log('is there a target id?', req.target );
		if( 'undefined' === typeof req.target || 'undefined' === typeof req.target.ids || 'group' === req.target.type ) {
	...
	var res = Private.request.post( Private.service.linkedin, url, token, secret, postbody, null, callback );
	console.log("RESSSSULT", res);
